from llm import get_gpt_knapsack_solution


def choose_llm():

    print("\n==============================\nChoose the Large Language Models: ")
    print("1 = Anthropic : claude 3.7 thinking")
    print("2 = OpenAI : o4 mini high")
    ...
    while True:
        try:
            ...
            if choice in [1, 2, 3]:
                return ["claude", "openai", "gemini"][choice - 1]
        except ValueError:
            pass
        print("Invalid choice.")


def run_experiments(ITEMS, CAPACITY):

    model_key = choose_llm()

    n = int(input("\nNumber of runs :"))

    results, solutions, times, costs = [], [], [], []

    for i in range(n):
        
        print(f"\n----------------------Iteration {i+1}/{n}----------------------")
        result = get_gpt_knapsack_solution(model_key, ITEMS, CAPACITY)
    ...
    print("\n------------------Final Statistics-----------------------")
    print(solutions)
    print(f"Avg solution: {sum(solutions)/n:.2f}")
    print(f"Avg time: {sum(times)/n:.2f}s")
...
run_experiments(ITEMS, KNAPSACK_CAPACITY)
